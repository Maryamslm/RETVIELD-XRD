...
_ALLOW = {
    "Fm-3m": _allow_fcc, "Im-3m": _allow_bcc,
    "P63/mmc": _allow_hcp, "P42/mnm": _allow_sig, "R-3m": _allow_all, "Pm-3m": _allow_all,
}
...
def generate_reflections(ph: Phase, wl: float = 1.54056,
                         tt_min: float = 10.0, tt_max: float = 100.0, n: int = 7) -> List[Dict]:
        ...
        for k in range(-n, n+1):
            for l in range(-n, n+1):
                ...
                if dk in seen:
                    ...
                    seen[dk] = {"h":h, "k":k, "l":l, "hkl_list":[(h,k,l)], "d":d, "tt":tt, "mult":1}
...
def _make_refined_phase(ph: Phase, a_ref: float, c_ref: float) -> Phase:
    return Phase(key=ph.key, name=ph.name, formula=ph.formula, pdf_card=ph.pdf_card,
                 ...
                 alpha=ph.alpha, beta=ph.beta, gamma=ph.gamma, atoms=ph.atoms, color=ph.color)
...
def caglioti(tt_deg: float, U: float, V: float, W: float) -> float:
    ...
    return np.sqrt(max(U*np.tan(th)**2 + V*np.tan(th) + W, 1e-8))
...
def phase_pattern(tt: np.ndarray, ph: Phase, a: float, c: float, scale: float,
                  U: float, V: float, W: float, eta0: float, z_shift: float, wl: float) -> np.ndarray:
...
class RietveldRefiner:
    def __init__(self, tt: np.ndarray, I_obs: np.ndarray, phase_keys: List[str],
                 wavelength: float = 1.54056, n_bg: int = 5):
    ...
    def _bounds(self, flags: Dict[str, bool]):
        ...
        for i, ph in enumerate(self.phases):
            ...
            if flags.get("lattice", True):
                free(b+1, ph.a*0.95, ph.a*1.05)
                free(b+2, ph.c*0.95, ph.c*1.05)
            else: freeze(b+1); freeze(b+2)
            if flags.get("profile", True):
                free(b+3, 0.0, 0.5); free(b+4, -0.1, 0.0)
                free(b+5, 1e-4, 0.1); free(b+6, 0.0, 1.0)
            else:
    ...
    def refine(self, flags: Dict[str, bool], max_iter: int = 400) -> Dict:
        ...
        return {**rf, "Icalc": Icalc, "Ibg": bg, "contribs": contribs,
                "diff": self.Iobs-Icalc, "wf": wf, "lat": lat, "z_shift": z}
...
def make_demo_pattern(noise: float = 0.025, seed: int = 7):
    ...
    for key, wf in wf_demo.items():
        ph = PHASE_DB[key]
        I += phase_pattern(tt, ph, ph.a, ph.c, wf*7500, 0.025, -0.012, 0.006, 0.45, 0.0, 1.54056)
    I = np.maximum(I, 0.0)
    I = rng.poisson(I).astype(float) + rng.normal(0, noise*I.max(), size=I.shape)
...
def parse_file_content(content: str, filename: str) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if name.endswith(".xrdml"):
        try:
            ...
            tt = np.linspace(start_pos or 10.0, end_pos or 100.0, len(I))
            return tt, I
    ...
    for filename in possible_files:
        if filename.endswith(file_ext):
            ...
            try:
                ...
                tt, I = parse_file_content(response.text, filename)
    ...
    if fetch_btn:
        with st.spinner(f"Fetching {selected_sample} from GitHub..."):
            try:
                ...
                st.info("💡 Try switching file format or check internet connection.")
...
if run and tt_raw is not None and sel_keys:
    ...
    tt_c, I_c = tt_raw[mask], I_raw[mask]
    if len(tt_c) < 50:
        st.error("Too few data points — widen the 2θ window.")
    else:
...
with tab_fit:
    if st.session_state["results"] is None:
        if tt_raw is not None:
            ...
            fig.update_layout(template="plotly_dark", paper_bgcolor="#030712", plot_bgcolor="#030712",
                            xaxis_title="2θ (°)", yaxis_title="Intensity (counts)", height=350, margin=dict(l=60,r=20,t=20,b=50))
        ...
        for i, ph_obj in enumerate(refiner.phases):
            ...
            fig.add_trace(go.Scatter(x=[p["tt"]+z_shift for p in pks], y=[y_tick]*len(pks), mode="markers",
                ...
                name=f"{ph_obj.name} hkl", showlegend=False), row=1, col=1)
        ...
        fig.update_layout(template="plotly_dark", paper_bgcolor="#030712", plot_bgcolor="#030712", height=650,
            legend=dict(bgcolor="#080e1a", bordercolor="#1e293b", font=dict(size=11), x=1.01, y=1),
...
with tab_phase:
    if st.session_state["results"] is None: st.info("Run refinement first.")
        ...
        c1, c2 = st.columns(2)
        with c1:
            fig_pie = go.Figure(go.Pie(labels=ph_names, values=wf_pct, hole=0.58, marker=dict(colors=ph_cols, line=dict(color="#030712", width=2.5)),
                textinfo="label+percent", textfont=dict(size=11.5, family="IBM Plex Sans"), hovertemplate="<b>%{label}</b><br>%{value:.2f} wt%<extra></extra>"))
        ...
        with c2:
            ...
            fig_bar = go.Figure(go.Bar(x=[wf_pct[i] for i in idx], y=[ph_names[i] for i in idx], orientation="h",
                marker=dict(color=[ph_cols[i] for i in idx]), text=[f"{wf_pct[i]:.2f} %" for i in idx], textposition="inside", insidetextfont=dict(color="white", size=11, family="IBM Plex Mono")))
...
with tab_report:
    if st.session_state["results"] is None: st.info("Run refinement first.")
        ...
        md_lines = []
        md_lines.append("# Rietveld Refinement Report")
        md_lines.append("## Co-Cr Dental Alloy · XRD Phase Analysis")
        md_lines.append(f"**Date:** {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M')}")
        md_lines.append(f"**Radiation:** {wl_label} · λ = {wavelength:.5f} Å")
        md_lines.append(f"**Data points:** {len(tt)}")
        md_lines.append(f"**2θ range:** {tt.min():.2f}° – {tt.max():.2f}°")
        md_lines.append(f"**Computation time:** {elapsed:.2f} s")
        md_lines.append("---")
        md_lines.append("## 1 · Refinement Quality")
        md_lines.append("| Indicator | Value | Guidance |")
        md_lines.append("|-----------|-------|----------|")
        md_lines.append(f"| R_wp | **{r['Rwp']*100:.2f} %** | < 10 % acceptable; < 5 % excellent |")
        md_lines.append(f"| R_p | **{r['Rp']*100:.2f} %** | — |")
        md_lines.append(f"| χ² | **{r['chi2']:.4f}** | — |")
        md_lines.append(f"| GOF | **{r['GOF']:.4f}** | ≈ 1 ideal |")
        md_lines.append("---")
        md_lines.append("## 2 · Phase Weight Fractions (Hill-Howard)")
        md_lines.append("| Phase | Formula | S.G. | wt % |")
        md_lines.append("|-------|---------|------|------|")
        for k, wf in r["wf"].items():
            ph = PHASE_DB[k]
            md_lines.append(f"| {ph.name} | {ph.formula} | {ph.space_group} | **{wf*100:.2f}** |")
        md_lines.append("---")
        md_lines.append("## 3 · Refined Lattice Parameters")
        md_lines.append("| Phase | a_init (Å) | a_ref (Å) | Δa | c_init (Å) | c_ref (Å) | Δc |")
        md_lines.append("|-------|-----------|-----------|-----|-----------|-----------|----|")
        for k, lp in r["lat"].items():
            ph = PHASE_DB[k]
            if ph.c != ph.a:
                md_lines.append(f"| {ph.name} | {lp['a_init']:.4f} | {lp['a_ref']:.4f} | {lp['da']:+.4f} | {lp['c_init']:.4f} | {lp['c_ref']:.4f} | {lp['dc']:+.4f} |")
            else:
                md_lines.append(f"| {ph.name} | {lp['a_init']:.4f} | {lp['a_ref']:.4f} | {lp['da']:+.4f} | — | — | — |")
        md_lines.append("---")
        md_lines.append("## 4 · Profile Parameters")
        md_lines.append("| Phase | Scale | U | V | W | η |")
        md_lines.append("|-------|-------|---|---|---|---|")
        for k, lp in r["lat"].items():
            ph = PHASE_DB[k]
            md_lines.append(f"| {ph.name} | {lp['scale']:.3e} | {lp['U']:.5f} | {lp['V']:.5f} | {lp['W']:.5f} | {lp['eta']:.3f} |")
        md_lines.append("---")
        md_lines.append("## 5 · Methodology")
        md_lines.append("- **Profile:** Thompson-Cox-Hastings pseudo-Voigt · Caglioti FWHM (U, V, W, η)")
        md_lines.append(f"- **Background:** Chebyshev polynomial ({n_bg} terms)")
        md_lines.append("- **LP correction:** Graphite monochromator (2θ_mono = 26.6°)")
        md_lines.append("- **Structure factors:** Cromer-Mann + isotropic Debye-Waller")
        md_lines.append("- **Weight fractions:** Hill-Howard formula")
        md_lines.append(f"- **Optimiser:** scipy.optimize.least_squares (TRF / Levenberg-Marquardt · {max_it} iter.)")
        md_lines.append("---")
        md_lines.append("## 6 · References")
        md_lines.append("1. Rietveld (1969) *J. Appl. Cryst.* **2**, 65–71")
        md_lines.append("2. Hill & Howard (1987) *J. Appl. Cryst.* **20**, 467–474")
        md_lines.append("3. Thompson et al. (1987) *J. Appl. Cryst.* **20**, 79–83")
        md_lines.append("4. Takaichi et al. (2013) *Acta Biomaterialia* **9**, 7901–7910")
        md_lines.append("5. ISO 22674:2016 · ASTM F75-18")
        md = "\n".join(md_lines)
        st.markdown(md)
        st.download_button("⬇ Download Report (.md)", data=md, file_name="rietveld_report.md", mime="text/markdown")
