...
║   Single-file Streamlit Application  (fixed)                    ║
║                                                                  ║
...
# ═══════════════════════════════════════════════════════════════════
# PARAMETER VECTOR
# layout: [z_shift, bg_0..bg_{n_bg-1},
#          scale_0,a_0,c_0,U_0,V_0,W_0,eta_0,
#          scale_1,a_1,c_1,U_1,V_1,W_1,eta_1, ...]
# ═══════════════════════════════════════════════════════════════════
...
class RietveldRefiner:
    ...
    def __init__(self, tt: np.ndarray, I_obs: np.ndarray,
        ...
        self.n_bg   = int(n_bg)
        self.phases = [PHASE_DB[k] for k in phase_keys]   # List[Phase]
...
# SESSION STATE
# ═══════════════════════════════════════════════════════════════════
for _k in ("results", "refiner", "tt", "Iobs", "elapsed"):
...
with st.sidebar:
    ...
    if src.startswith("Demo"):
        ...
        st.info("Synthetic SLM Co-Cr-Mo · Cu Kα₁ · λ = 1.5406 Å\n\n"
                "68 % γ-Co · 15 % ε-Co · 8 % σ · 5 % Cr · 4 % Mo")
    else:
        up = st.file_uploader(
            "Drag & drop XRD file",
            type=["xy","dat","txt","csv","xrdml"],
            label_visibility="collapsed",
            help="Two-column text or Panalytical .xrdml",
        )
        if up:
            try:
...
with tab_fit:
    ...
    if st.session_state["results"] is None:
        ...
        # unpack refined params once — used for tick marks
        ...
        for key, Iph in r["contribs"].items():
            ph  = PHASE_DB[key]          # key is a str — correct
        ...
        # hkl tick marks — iterate over refiner.phases (List[Phase])
        y_tick = float(Iobs.min()) - 0.05*(float(Iobs.max()) - float(Iobs.min()))
        for i, ph_obj in enumerate(refiner.phases):           # ph_obj is a Phase
...
with tab_peaks:
    if st.session_state["refiner"] is None:
        ...
        rows = []
        for i, ph_obj in enumerate(refiner.phases):    # ph_obj is Phase
...
with tab_params:
    if st.session_state["results"] is None:
        ...
        lp_rows = []
        for ph_obj in refiner.phases:                   # ph_obj is Phase
...
```
1. hkl reflections  — systematic-absence rules per space group
2. d-spacing        — Bragg's law (cubic / hexagonal / tetragonal)
3. |F(hkl)|²        — Cromer-Mann factors + isotropic Debye-Waller
4. Profile          — Thompson-Cox-Hastings pseudo-Voigt
   FWHM             — Caglioti: sqrt(U·tan²θ + V·tanθ + W)
5. Background       — Chebyshev polynomial (2–8 terms)
6. LP correction    — graphite monochromator geometry
7. Minimise R_wp    — scipy least_squares (TRF / Levenberg-Marquardt)
8. Weight fractions — Hill-Howard formula
```
...
<p style="text-align:center;color:#1e293b;font-size:.72rem;margin-top:6px;">
  Co-Cr XRD Rietveld &nbsp;·&nbsp;
  Full-profile phase analysis for dental alloys &nbsp;·&nbsp;
