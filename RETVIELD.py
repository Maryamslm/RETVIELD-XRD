...
GITHUB_RAW_BASE =
f"https://raw.githubusercontent.com/{GITHUB_REPO}/{GITHUB_COMMIT}/samples/"
...
themes = {
"Dark Mode": {"bg": "#020617", "text": "#e2e8f0", "sidebar": "#030712", "panel": "#080e1a", "border":
"#1e293b"},
"Light Mode": {"bg": "#f8fafc", "text": "#0f172a", "sidebar": "#ffffff", "panel": "#f1f5f9", "border":
"#cbd5e1"},
"High Contrast": {"bg": "#000000", "text": "#00ff00", "sidebar": "#0a0a0a", "panel": "#111111", "border":
"#00ff0044"}
}
...
family=IBM+Plex+Mono:wght@400;600&family=IBM+Plex+Sans:wght@300;400;600;700&display=swap');
html, body, [class*="css"] {{ font-family: 'IBM Plex Sans', sans-serif !important; font-size: {font_size}rem
!important; }}
code, pre {{ font-family: 'IBM Plex Mono', monospace !important; }}

[data-testid="stAppViewContainer"] > .main {{ background-color: {t['bg']} !important; color: {t['text']}
!important; }}
[data-testid="stHeader"] {{ background: transparent; }}
[data-testid="stSidebar"] {{ background: {t['sidebar']} !important; border-right: 1px solid {t['border']}; }}
[data-testid="stSidebar"] * {{ color: {t['text']} !important; }}
[data-testid="stSidebar"] .stSlider label, [data-testid="stSidebar"] .stCheckbox label {{ color: #94a3b8
!important; }}
.stButton > button {{ border-radius: 8px !important; font-weight: 600 !important; letter-spacing: .03em
!important; }}
.stButton > button[kind="primary"] {{ background: linear-gradient(135deg, {primary_color}, #7c3aed)
!important; border: none !important; color: white !important; }}
.hero {{ background: linear-gradient(135deg, {t['bg']} 0%, {t['panel']} 45%, {t['bg']} 100%); border: 1px
solid {t['border']}; border-radius: 14px; padding: 28px 36px 22px; margin-bottom: 22px; position: relative;
overflow: hidden; }}

.hero h1 {{ font-size: 1.9rem; font-weight: 700; letter-spacing: -.02em; background: linear-
gradient(100deg, {primary_color} 0%, #818cf8 50%, #34d399 100%); -webkit-background-clip: text; -webkit-
text-fill-color: transparent; margin: 0 0 6px; }}

.hero p {{ color: #64748b; margin: 0; font-size: .88rem; font-weight: 300; line-height: 1.5; }}
.badge {{ display: inline-block; font-size: .7rem; font-weight: 600; letter-spacing: .06em; padding: 2px
9px; border-radius: 99px; margin-right: 6px; margin-top: 10px; border: 1px solid; }}
...
.mc {{ background: {t['panel']}; border: 1px solid {t['border']}; border-radius: 10px; padding: 12px 18px;
flex: 1; min-width: 110px; }}
.mc .lbl {{ font-size: .68rem; color: #475569; letter-spacing: .1em; text-transform: uppercase; }}
.mc .val {{ font-size: 1.45rem; font-weight: 700; color: {t['text']}; font-family: 'IBM Plex Mono',
monospace; }}
.mc .sub {{ font-size: .7rem; color: #334155; }}
.sh {{ font-size: .7rem; font-weight: 700; letter-spacing: .14em; text-transform: uppercase; color:
#334155; border-bottom: 1px solid {t['border']}; padding-bottom: 4px; margin: 16px 0 10px; }}
</style>
...
db: Dict[str, Phase] = {}
db["gamma_Co"] = Phase(key="gamma_Co", name="γ-Co (FCC)", formula="Co", pdf_card="PDF 15-
0806",
...
db["epsilon_Co"] = Phase(key="epsilon_Co", name="ε-Co (HCP)", formula="Co", pdf_card="PDF 05-
0727",
crystal_system="hexagonal", space_group="P63/mmc", sg_number=194, a=2.5071, b=2.5071,
c=4.0686,
...
crystal_system="tetragonal", space_group="P42/mnm", sg_number=136, a=8.7960, b=8.7960,
c=4.5750,
...
description="Cr-rich intermetallic; appears after prolonged heat treatment or slow cooling; tetragonal
structure.")
...
crystal_system="hexagonal", space_group="P63/mmc", sg_number=194, a=5.1400, b=5.1400,
c=4.1000,
...
db["M23C6"] = Phase(key="M23C6", name="M23C6 Carbide", formula="Cr23C6", pdf_card="PDF 36-
0803",
...
description="Cr23C6 type; very common in cast alloys with carbon; detected by characteristic low-
angle peaks.")
...
AtomSite("C", "48f", 0.375, 0.375, 0.375, 1.0, 0.50)], wf_init=0.05, color="#f97316",
group="Carbides",
description="Mo/W-rich; found in Mo- or W-containing alloys.")
db["Laves"] = Phase(key="Laves", name="Laves Phase (Co2Mo)", formula="Co2Mo", pdf_card="PDF
03-1225",
...
db["Cr2O3"] = Phase(key="Cr2O3", name="Cr2O3 (Eskolaite)", formula="Cr2O3", pdf_card="PDF 38-
1479",
...
db["CoCr2O4"] = Phase(key="CoCr2O4", name="CoCr2O4 (Spinel)", formula="CoCr2O4",
pdf_card="PDF 22-1084",
crystal_system="cubic", space_group="Fm-3m", sg_number=227, a=8.3216, b=8.3216, c=8.3216,
atoms=[AtomSite("Co", "8a", 0.125, 0.125, 0.125, 1.0, 0.55), AtomSite("Cr", "16d", 0.5, 0.5, 0.5, 1.0,
0.55),
AtomSite("O", "32e", 0.264, 0.264, 0.264, 1.0, 0.65)], wf_init=0.01, color="#22d3ee",
group="Oxide",
...
def generate_reflections(ph: Phase, wl: float = 1.54056, tt_min: float = 10.0, tt_max: float = 100.0, n: int = 7)
-> List[Dict]:
...
def phase_pattern(tt: np.ndarray, ph: Phase, a: float, c: float, scale: float, U: float, V: float, W: float, eta0:
float, z_shift: float, wl: float) -> np.ndarray:
...
═══════════════════════════════════════════════════════════════════
N_PP = 7
def _pack(z, bg, per_phase) -> np.ndarray: return np.array([z, *bg, *[v for p in per_phase for v in p]],
dtype=float)
...
scale = float(p[0]); uc_mass = sum(_MASS.get(at.element, 50.0) * at.occupancy for at in ph.atoms) or
1.0
...
def __init__(self, tt: np.ndarray, I_obs: np.ndarray, phase_keys: List[str], wavelength: float = 1.54056,
n_bg: int = 5):
...
if flags.get("profile", True): free(b+3, 0.0, 0.5); free(b+4, -0.1, 0.0); free(b+5, 1e-4, 0.1); free(b+6, 0.0,
1.0)
else:
...
return {**rf, "Icalc": Icalc, "Ibg": bg, "contribs": contribs, "diff": self.Iobs-Icalc, "wf": wf, "lat": lat, "z_shift":
z}
...
ph = PHASE_DB[key]; I += phase_pattern(tt, ph, ph.a, ph.c, wf*7500, 0.025, -0.012, 0.006, 0.45, 0.0,
1.54056)
...
try: start_pos = float(pos_elem.find("{*}startPosition").text); end_pos = float(pos_elem.find("
...
try: response = requests.get(url, timeout=30); response.raise_for_status(); return
parse_file_content(response.text, filename) + (filename,)
except: continue
for filename in possible_files:
url = GITHUB_RAW_BASE + filename
try: response = requests.get(url, timeout=30); response.raise_for_status(); return
...
hkl_label_offset = st.slider("Label vertical offset (%)", 0, 50, 15, help="Offset labels above peaks to avoid
overlap")
...
st.success(source_info); st.session_state.selected_sample = selected_sample;
st.session_state.source_info = source_info
except Exception as e: st.error(f"❌ Error: {str(e)}"); st.info("💡 Try switching file format or check
internet connection.")
with st.expander("🔁 Alternative: Demo or Local Upload", expanded=False):
src = st.radio("", ["Demo pattern (synthetic)", "Upload my XRD file"], label_visibility="collapsed",
key="alt_src")
if src.startswith("Demo"):
tt_raw, I_raw = make_demo_pattern(); st.info("Synthetic SLM Co-Cr-Mo · Cu Kα1"); source_info =
"Demo pattern loaded"
else:
up = st.file_uploader("Drag & drop XRD file", type=["xy","dat","txt","csv","xrdml","asc"],
...
WL_OPTIONS = {"Cu Kα1 (1.54056 Å)": 1.54056, "Cu Kα (1.54184 Å)": 1.54184, "Mo Kα1 (0.70932 Å)":
0.70932, "Ag Kα1 (0.56087 Å)": 0.56087, "Co Kα1 (1.78900 Å)": 1.78900}
wl_label = st.selectbox("Wavelength", list(WL_OPTIONS.keys()), index=0); wavelength =
WL_OPTIONS[wl_label]
...
("Carbides", CARBIDE_KEYS, True), ("Laves Phase", LAVES_KEYS, True), ("Oxide phases",
OXIDE_KEYS, False),
]
...
fl_zero = st.checkbox("Zero-shift", value=False); n_bg = st.slider("Background terms", 2, 8, 5); max_it =
st.slider("Max iterations", 50, 1000, 350, 50)
st.markdown("")
run = st.button("▶ Run Rietveld Refinement", type="primary", use_container_width=True, disabled=
(tt_raw is None or not sel_keys))
...
st.caption(f"📊 Current sample: **{st.session_state.selected_sample}** —
...
"📈 Pattern Fit", "⚖️ Phase Analysis", "📋 Peak List", "🔧 Refined Parameters", "📄 Report", "i️
About"])
...
r, refiner, tt, Iobs, elapsed = st.session_state["results"], st.session_state["refiner"], st.session_state["tt"],
st.session_state["Iobs"], st.session_state["elapsed"]
...
fig.add_trace(go.Scatter(x=tt, y=Iobs, mode="lines", name="I_obs (exp)", line=dict(color="#94a3b8",
width=1.3)), row=1, col=1)
fig.add_trace(go.Scatter(x=tt, y=r["Ibg"], mode="lines", name="Background",
line=dict(color="#334155", width=1, dash="dot"), fill="tozeroy", fillcolor="rgba(51,65,85,0.12)"), row=1,
col=1)
...
fig.add_trace(go.Scatter(x=tt, y=r["Icalc"], mode="lines", name="I_calc", line=dict(color="#fbbf24",
width=2.2)), row=1, col=1)
...
marker=dict(symbol="line-ns", size=11, color=ph_obj.color, line=dict(width=2,
...
fig.add_trace(go.Scatter(x=tt, y=r["diff"], mode="lines", name="Δ obs−calc", line=dict(color="#818cf8",
width=1), fill="tozeroy", fillcolor="rgba(129,140,248,0.12)"), row=2, col=1)
...
ph_names, wf_pct, ph_cols = [PHASE_DB[k].name for k in wf_dict], [wf_dict[k]*100 for k in wf_dict],
[PHASE_DB[k].color for k in wf_dict]
c1, c2 = st.columns(2)
with c1:
...
textfont=dict(size=11.5, family="IBM Plex Sans"), hovertemplate="<b>%{label}</b><br>%{value:.2f} wt%
<extra></extra>"))
fig_pie.add_annotation(text="Weight<br>Fraction", x=0.5, y=0.5, font=dict(size=12,
color="#64748b"), showarrow=False)
fig_pie.update_layout(template=plot_theme, showlegend=False, height=360,
margin=dict(l=10,r=10,t=30,b=10), title=dict(text="Phase Composition", font=dict(size=13,
color="#64748b")))
...
rows.append({"Phase": ph.name, "Formula": ph.formula, "PDF Card": ph.pdf_card, "System":
ph.crystal_system.title(), "S.G.": ph.space_group, "wt %": f"{wf*100:.2f}", "a ref (Å)": f"{lp.get('a_ref',
ph.a):.4f}", "c ref (Å)": f"{lp.get('c_ref', ph.c):.4f}" if ph.c != ph.a else "—", "Group": ph.group})
...
if wm["gamma_Co"] > 50: msgs.append(("✅","γ-Co (FCC)",f"{wm['gamma_Co']:.1f}
wt%","Dominant austenitic matrix — typical of rapid-solidification SLM. Good biocompatibility and ductility
for dental prostheses."))
if wm["epsilon_Co"] > 5: msgs.append(("⚠️","ε-Co (HCP)",f"{wm['epsilon_Co']:.1f} wt%","Elevated
HCP fraction — martensitic transformation driven by residual stress or strain. Monitor fatigue
performance."))
if wm["sigma"] > 3: msgs.append(("🔴","σ-phase",f"{wm['sigma']:.1f} wt%","Brittle intermetallic —
risk to ductility and fracture toughness. Consider solution anneal at >1100 °C + water quench."))
if wm["Cr_bcc"] > 2: msgs.append(("🔵","Cr (BCC)",f"{wm['Cr_bcc']:.1f} wt%","Free chromium —
may indicate incomplete alloying or Cr-rich segregation."))
if wm["Mo_bcc"] > 1: msgs.append(("🟣","Mo (BCC)",f"{wm['Mo_bcc']:.1f} wt%","Segregated
molybdenum — inter-dendritic artefact; homogenisation anneal at 1150 °C / 1 h recommended."))
if wm.get("Cr2O3",0) > 0.5 or wm.get("CoCr2O4",0) > 0.5: msgs.append(("🟠","Oxides",f"Cr2O3=
{wm.get('Cr2O3',0):.1f}% CoCr2O4={wm.get('CoCr2O4',0):.1f}%","Surface/internal oxide detected — check
SLM atmosphere and powder storage."))
if not msgs: msgs.append(("i️","No dominant phases","—","Check phase selection and 2θ range."))
for icon, name, pct, msg in msgs:

st.markdown(f"""<div style="background:#080e1a;border:1px solid {border_color};border-
...
<div style="color:#94a3b8;font-size:.85rem;line-height:1.55;">{msg}</div></div>""",
...
z_shift = float(r.get("z_shift", 0.0)); _, _, pp_vec = _unpack(refiner.x0, refiner.n_bg, refiner.n_ph)
show = st.multiselect("Phases to display", options=[ph.key for ph in refiner.phases], default=[ph.key for
ph in refiner.phases], format_func=lambda k: PHASE_DB[k].name)
rows = []
for i, ph_obj in enumerate(refiner.phases):
if ph_obj.key not in show: continue
a_ref, c_ref = float(pp_vec[i][1]), float(pp_vec[i][2]); ph_ref = _make_refined_phase(ph_obj, a_ref,
c_ref)

for ref in generate_reflections(ph_ref, wl=wavelength, tt_min=float(tt.min()), tt_max=float(tt.max())):
h, k, l = ref["h"], ref["k"], ref["l"]
rows.append({"Phase": ph_obj.name, "hkl": f"({h} {k} {l})", "d (Å)": f"{ref['d']:.4f}", "2θ (°)": f"{ref['tt']
+ z_shift:.3f}", "Mult.": ref["mult"], "Group": ph_obj.group})
if rows:
...
r, refiner, lat, tt = st.session_state["results"], st.session_state["refiner"], st.session_state["results"]["lat"],
st.session_state["tt"]
st.markdown('<div class="sh">Lattice Parameters</div>', unsafe_allow_html=True)
lp_rows = []
for ph_obj in refiner.phases:
lp = lat.get(ph_obj.key, {})
lp_rows.append({"Phase": ph_obj.name, "System": ph_obj.crystal_system.title(), "S.G.":
ph_obj.space_group,
"a_init (Å)": f"{lp.get('a_init', ph_obj.a):.4f}", "a_ref (Å)": f"{lp.get('a_ref', ph_obj.a):.4f}", "Δa (Å)": f"
{lp.get('da', 0):+.4f}",
...
'.join(f'{float(v):.2f}' for v in bg_c)} (Chebyshev coeff. 0–{len(bg_c)-1})", language="text")
...
r, refiner, elapsed, tt = st.session_state["results"], st.session_state["refiner"],
st.session_state["elapsed"], st.session_state["tt"]
md = []
md.append("# Rietveld Refinement Report"); md.append("## Co-Cr Dental Alloy · XRD Phase
Analysis")
md.append(f"**Date:** {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M')}");
md.append(f"**Radiation:** {wl_label} · λ = {wavelength:.5f} Å")
md.append(f"**Data points:** {len(tt)}"); md.append(f"**2θ range:** {tt.min():.2f}° – {tt.max():.2f}°");
md.append(f"**Computation time:** {elapsed:.2f} s")
md.append("---"); md.append("## 1 · Refinement Quality"); md.append("| Indicator | Value | Guidance
|"); md.append("|-----------|-------|----------|")
md.append(f"| R_wp | **{r['Rwp']*100:.2f} %** | < 10 % acceptable; < 5 % excellent |"); md.append(f"|
R_p | **{r['Rp']*100:.2f} %** | — |")
md.append(f"| χ2 | **{r['chi2']:.4f}** | — |"); md.append(f"| GOF | **{r['GOF']:.4f}** | ≈ 1 ideal |")
md.append("---"); md.append("## 2 · Phase Weight Fractions (Hill-Howard)"); md.append("| Phase |
Formula | S.G. | wt % |"); md.append("|-------|---------|------|------|")
for k, wf in r["wf"].items():
ph = PHASE_DB[k]; md.append(f"| {ph.name} | {ph.formula} | {ph.space_group} | **{wf*100:.2f}** |")
md.append("---"); md.append("## 3 · Refined Lattice Parameters"); md.append("| Phase | a_init (Å) |
a_ref (Å) | Δa | c_init (Å) | c_ref (Å) | Δc |"); md.append("|-------|-----------|-----------|-----|-----------|-----------|----|")
for k, lp in r["lat"].items():
ph = PHASE_DB[k]
if ph.c != ph.a: md.append(f"| {ph.name} | {lp['a_init']:.4f} | {lp['a_ref']:.4f} | {lp['da']:+.4f} |
{lp['c_init']:.4f} | {lp['c_ref']:.4f} | {lp['dc']:+.4f} |")
else: md.append(f"| {ph.name} | {lp['a_init']:.4f} | {lp['a_ref']:.4f} | {lp['da']:+.4f} | — | — | — |")
md.append("---"); md.append("## 4 · Profile Parameters"); md.append("| Phase | Scale | U | V | W | η
|"); md.append("|-------|-------|---|---|---|---|")
for k, lp in r["lat"].items():
ph = PHASE_DB[k]; md.append(f"| {ph.name} | {lp['scale']:.3e} | {lp['U']:.5f} | {lp['V']:.5f} | {lp['W']:.5f} |
{lp['eta']:.3f} |")
...
md.append(f"- **Optimiser:** scipy.optimize.least_squares (TRF / Levenberg-Marquardt · {max_it}
iter.)")
...
md_text = "\n".join(md)
st.markdown(md_text)
st.download_button("⬇ Download Report (.md)", data=md_text, file_name="rietveld_report.md",
mime="text/markdown")

with tab_about:
about_lines = [
"## About", "", "Full-profile **Rietveld refinement** for X-ray diffraction patterns from",
"**3D-printed (SLM/DMLS) Co-Cr dental alloys** — entirely in the browser.", "", "### ✨ GitHub
Integration",
"- **Dropdown selector** for samples: `CH0`, `CH45`, `CNH0`, `CNH45`, `PH0`, `PH45`, `PNH0`,
`PNH45`",
...
"| `.CSV` | Comma-separated | Header optional |", "| `.XRDML` | Panalytical XML | Native
PANalytical/X'Pert format |",
...
"3. Thompson et al. (1987) *J. Appl. Cryst.* **20**, 79", "4. Takaichi et al. (2013) *Acta Biomater.* **9**,
7901",
